...
1. **증권 인증은 사용자가 직접 수행합니다.** `kis auth login`, `kis auth test`, `kis auth logout` 등 인증 계열 명령은 **에이전트가 대신 실행하면 안 됩니다**. APP_KEY / APP_SECRET / 계좌번호 같은 민감 정보는 사용자의 로컬 터미널에서 hidden prompt 로만 입력되어야 합니다. 에이전트는 명령어를 **안내만** 하고 사용자의 완료 보고를 기다리세요. 사용자가 그 값을 채팅으로 보내려고 하면 정중히 중단시키세요.
2. **모의계정 우선.** 모든 주문 명령에는 기본적으로 `--profile paper` 를 붙이세요. 사용자가 명시적으로 "실전" 또는 "production" 이라고 말한 경우에만 `--profile prod` 를 씁니다. 모호한 "매수해줘" 는 **paper** 로 해석합니다.
3. **주문 전 금액 확인.** `kis order buy/sell` 실행 전에 반드시 plan(종목, 수량, 가격, 매수/매도, 프로파일) 을 사용자에게 요약해서 돌려주고 "yes" 를 받은 뒤, `-y` 플래그로 CLI 내부 프롬프트만 건너뛰세요.
4. **인증 정보 노출 금지.** `~/.kis-cli/config.yaml` 이나 `~/.kis-cli/tokens.json` 의 내용을 절대 echo 하거나 채팅창에 출력하지 마세요.
5. **연쇄 추론은 `--json` 으로.** 결과를 이어서 다음 명령의 입력으로 쓸 계획이라면, 무조건 `--json` 을 붙여 구조화된 출력을 받으세요.
6. **rate limit.** 모의투자 환경은 호출 사이에 500ms 강제 대기가 있습니다(CLI 가 자동 처리). 그래도 연속 ~10회 이상을 쉬지 않고 쏘지는 마세요.
...
응답 JSON 의 `output.stck_prpr` (현재가), `output.prdy_vrss` (전일대비), `output.prdy_ctrt` (전일 대비율) 를 파싱해서 한국어로 요약하세요.
...
**중요:** 실제 주문 실행은 **사용자가 직접** 터미널에서 해주셔야 합니다. 계정 자산을 움직이는 명령이므로 에이전트가 대신 실행하지 않습니다. 에이전트는 명령어 제안까지만 합니다.

1. 매수가능 여부를 확인하도록 사용자에게 명령을 안내 (에이전트 실행 X, 사용자 직접 실행):
   ```
   kis balance orderable --symbol 005930 --price 70000 --profile paper --json
   ```
   사용자로부터 JSON 출력(특히 `output.ORD_PSBL_CASH`, `output.MAX_BUY_AMT`) 을 받습니다.
2. 계획을 한국어로 요약해서 사용자에게 확인받습니다.
   ...
   >
   > 이대로 진행하시겠습니까? (yes/no)"
3. "yes" 답변을 받으면 **실행 명령을 그대로 보여주고 사용자가 직접 실행하도록 안내**:
   ```
   아래 명령을 터미널에서 직접 실행해주세요:

      kis order buy --symbol 005930 --qty 1 --price 70000 --profile paper

   CLI 가 '이대로 주문을 전송할까요?' 라고 한 번 더 물어보면 y 를 입력하세요.
   실행 결과(특히 output.ODNO) 를 알려주시면 다음 단계를 도와드리겠습니다.
   ```
4. 사용자가 응답의 `output.KRX_FWDG_ORD_ORGNO` 와 `output.ODNO` 를 알려주시면, 이후 정정/취소 시 사용할 수 있도록 기억해두세요.
...
정정 / 취소 실행 전에도 주문과 동일하게 plan 을 요약해 사용자에게 확인받고 `-y` 를 붙이세요.
...
| 에러 | 의미 | 에이전트가 할 일 |
|---|---|---|
| `Profile "paper" not found` | 로그인 안 됨 | `kis auth login --paper` 실행 후 재시도 |
| `KIS API error: EGW00113` | 토큰 만료 | `kis auth test --profile paper` 실행 후 재시도 |
...
상세 디버깅이 필요하면 `KIS_DEBUG=1` 을 접두사로 붙이세요.

...
1. **모의계정 우선.** 모든 주문 명령에는 기본적으로 `--profile paper` 를 붙이세요. 사용자가 명시적으로 "실전" 또는 "production" 이라고 말한 경우에만 `--profile prod` 를 씁니다. 모호한 "매수해줘" 는 **paper** 로 해석합니다.
2. **주문 전 금액 확인.** `kis order buy/sell` 실행 전에 반드시 plan(종목, 수량, 가격, 매수/매도, 프로파일) 을 사용자에게 요약해서 돌려주고 "yes" 를 받은 뒤, `-y` 플래그로 CLI 내부 프롬프트만 건너뛰세요.
3. **인증 정보 노출 금지.** `~/.kis-cli/config.yaml` 이나 `~/.kis-cli/tokens.json` 의 내용을 절대 echo 하거나 채팅창에 출력하지 마세요.
4. **연쇄 추론은 `--json` 으로.** 결과를 이어서 다음 명령의 입력으로 쓸 계획이라면, 무조건 `--json` 을 붙여 구조화된 출력을 받으세요.
5. **rate limit.** 모의투자 환경은 호출 사이에 500ms 강제 대기가 있습니다(CLI 가 자동 처리). 그래도 연속 ~10회 이상을 쉬지 않고 쏘지는 마세요.
...
1. 먼저 매수 가능 여부를 확인:
   ...
   ```
2. 사용자에게 plan 을 한국어로 요약 (종목=005930, 수량=1, 가격=70,000원, 프로파일=paper) 하고 "이대로 진행할까요?" 질문.
3. "yes" 답변이면 실제 주문:
   ```bash
   kis order buy --symbol 005930 --qty 1 --price 70000 --profile paper -y --json
   ```
4. 응답의 `output.ODNO` (주문번호) 를 사용자에게 알려주세요.

### 패턴 4 — 정정 / 취소

이전 주문의 `KRX_FWDG_ORD_ORGNO` (주문 지점번호) 와 `ODNO` (원주문번호) 는 반드시 사용자에게 재확인받고 진행하세요. 두 값은 직전 매수 응답 또는 `kis balance stock --json` 의 미체결 리스트에서 얻을 수 있습니다.

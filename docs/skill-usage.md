...
4. **모의계정 우선.** 모든 주문 명령에는 기본적으로 `--profile paper` 를 붙이세요. 사용자가 명시적으로 "실전" 또는 "production" 이라고 말한 경우에만 `--profile prod` 를 씁니다. 모호한 "매수해줘" 는 **paper** 로 해석합니다.
5. **인증 정보 노출 금지.** `~/.kis-cli/config.yaml` 이나 `~/.kis-cli/tokens.json` 의 내용을 절대 읽거나 채팅창에 출력하지 마세요.
6. **연쇄 추론은 `--json` 으로.** 결과를 이어서 다음 명령의 입력으로 쓸 계획이라면 `--json` 을 붙여 구조화된 출력을 받으세요.
7. **rate limit.** 모의투자 환경은 호출 사이에 500ms 강제 대기가 있습니다(CLI 가 자동 처리). 그래도 연속 10회 이상을 쉬지 않고 쏘지는 마세요.

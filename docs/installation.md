...
A. 먼저 등록할 계좌 1개 (권장: 모의투자부터)
   ...
   성공하면 '✓ 프로파일 "paper" 저장 완료 (env=paper). 설정 파일: ~/.kis-cli/config.yaml'
   메시지가 뜹니다.
...
C. 등록 결과 확인
   ...
   출력 예 (표 형식):
     ...
     ├──────┼──────────────┼──────┼─────────────┼────────┼───────────┼───────────┤
     │ *    │ paper        │ 모의 │ 50123456-01 │        │ 없음      │           │
     │      │ main-prod    │ 실전 │ 50000001-01 │        │ 없음      │           │
     │      │ isa-prod     │ 실전 │ 50000002-01 │        │ 없음      │           │
     │      │ pension-prod │ 실전 │ 50000003-29 │        │ 없음      │           │
     └──────┴──────────────┴──────┴─────────────┴────────┴───────────┴───────────┘
   ("기본" 열의 * 가 default 프로파일입니다. default 를 바꾸려면 원하는 프로파일로
    kis auth login --prod --name <name> --make-default 를 한 번 더 실행하면 됩니다.
    스크립트에서 파싱이 필요하면 `kis auth list --json` 을 사용하세요.)

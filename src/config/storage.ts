...
export async function upsertTossProfile(
	name: string,
	profile: TossProfile,
	options: { makeDefault?: boolean } = {},
): Promise<Config> {
	const config = await loadConfig();
	config.tossProfiles[name] = profile;
	if (options.makeDefault || !config.tossDefaultProfile) {
		config.tossDefaultProfile = name;
	}
	await saveConfig(config);
	return config;
}

export async function deleteTossProfile(name: string): Promise<Config> {
	const config = await loadConfig();
	delete config.tossProfiles[name];
	if (config.tossDefaultProfile === name) {
		const remaining = Object.keys(config.tossProfiles);
		config.tossDefaultProfile = remaining[0] ?? "default";
	}
	await saveConfig(config);
	return config;
}
...
export function getTossProfile(config: Config, name?: string): TossProfile {
	...
	if (!profile) {
		throw new Error(
			`Toss \ud504\ub85c\ud30c\uc77c \"${target}\"\uc744(\ub97c) \ucc3e\uc744 \uc218 \uc5c6\uc2b5\ub2c8\ub2e4. 'toss auth login'\uc73c\ub85c \uc0dd\uc131\ud558\uc138\uc694.`,
		);
	}
	return profile;
}

/** 토스 토큰 캐시 키 (KIS 키와 충돌하지 않도록 `toss:` prefix 사용). */
